	...
	"group/button inline-flex shrink-0 items-center justify-center rounded-lg border border-transparent bg-clip-padding text-sm whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
	{
		variants: {
			variant: {
				...
				outline:
					"border-foreground/15 bg-background hover:bg-background/60 hover:text-foreground aria-expanded:bg-background/60 aria-expanded:text-foreground",
				secondary:
				...
				link: "w-fit px-0! h-fit! text-primary underline decoration-transparent underline-offset-6 transition-[text-decoration-color] hover:decoration-primary",
			},
			size: {
				default:
...
function buttonVariants(
	args: { className?: string } & VariantProps<typeof _buttonVariants>,
) {
	return cn(
		_buttonVariants({ size: args.size, variant: args.variant }),
		args.className,
	);
}
...
function Button({
	...
	size = "default",
	...props
}: ButtonPrimitive.Props & VariantProps<typeof _buttonVariants>) {
	return (
		<ButtonPrimitive
			data-slot="button"
